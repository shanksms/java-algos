...
public class TimeMap {
  ...
  public String get(String key, int timestamp) {
    ...
    int result = Collections.binarySearch(list, new Pair<>(timestamp, "{"),
        Comparator
            .comparingInt(o -> o.getKey()));

    if (result >= 0) return list.get(result).getValue();
    else if (result == -1) {
      return "";
    } else {
      return list.get(-result - 2).getValue();
    }
